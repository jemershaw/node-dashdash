...
// ---- completion tempaltes
const BASH_COMPLETION_TEMPLATE_PATH = (function () {
  var shell = 'bash';
  switch (process.env.SHELL.split('/')[2]) {
    case 'zsh':
      shell = 'zsh'
      break;
    default:
  ...
  return  path.join(
    __dirname, '../etc/dashdash.'+shell+'_completion.in');
})();



const COMPLETION_DESCRIPTION = function(desc) {
  desc = desc || '';
...
function bashCompletionSpecFromOptions(args) {
    ...
    var spec = [];
    var shortopts = [];
    var shortoptsdescription = [];
    var longopts = [];
    var longoptsdescription = [];
    var optargs = [];
            ...
            optNames.forEach(function (optName) {
                if (optName.length === 1) {
                    if (includeHidden || !o.hidden) {
                        ...
                        desc = (optDescription) ? optDescription.replace(':','\:') : '';
                        shortoptsdescription.push(COMPLETION_DESCRIPTION(optDescription));
                    }
                    ...
                    if (includeHidden || !o.hidden) {
                        ...
                        longoptsdescription.push(COMPLETION_DESCRIPTION(optDescription));
                    }
                    optargs.push('--' + optName + '=' + completionType);
                }
    ...
    spec.push(format('local cmd%s_shortopts="%s"',
        context, shortopts.join(' ')));
    spec.push(format('local cmd%s_shortoptsdescription="%s"',
        context, shortoptsdescription.join('|')));
    spec.push(format('local cmd%s_longopts="%s"',
        context, longopts.join(' ')));
    spec.push(format('local cmd%s_longoptsdescription="%s"',
        context, longoptsdescription.join('|')));
    spec.push(format('local cmd%s_optargs="%s"',
        context, optargs.join(' ')));
    if (args.argtypes) {
